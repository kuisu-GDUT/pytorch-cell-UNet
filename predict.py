...
def get_args():
    ...
    parser.add_argument('--model_name', '-name', default='NestedUNet',
                        help='Net structure name of model')
...
if __name__ == '__main__':
    ...
    net = Model(n_channels=3, n_classes=2, bilinear=args.bilinear,
                deep_supervision=args.deep_supervision)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    logging.info(f'Loading model {args.model_weight}')
    logging.info(f'Using device {device}')

    net.to(device=device)
    net.load_state_dict(torch.load(args.model_weight, map_location=device))
