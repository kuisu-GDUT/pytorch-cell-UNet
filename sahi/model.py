...
class DetectionModelUNet:
    def __init__(
        self,
        model_name:str,
        model_path: str,
        config_path: Optional[str] = None,
        deep_superivision: bool = True,
        device: Optional[str] = None,
        mask_threshold: float = 0.5,
        confidence_threshold: float = 0.3,
        category_mapping: Optional[Dict] = None,
        ...
        self.model_path = model_path
        self.config_path = config_path
        self.deep_superivision = deep_superivision
        self.model = None
    ...
    def load_model(self):
        ...
        net = Model(n_channels=3, n_classes=2, bilinear=False,
                   deep_supervision=self.deep_superivision)
    ...
    def perform_inference(self, image: np.ndarray, image_size: int = None):
        ...
        Args:
            ...
            image_size: int
                Inference input size.
        """
        self._original_predictions = self.predict_img(net=self.model,
                           full_img=image,
                           scale_factor=1,
    ...
    def _create_object_prediction_list_from_original_predictions(
        ...
        Args:
            ...
            full_shape_list: list of list
                ...
                List[[height, width],[height, width],...]
        """
        # mask = self._original_predictions
        # if mask.ndim == 2:
        #     mask_result= (mask * 255).astype(np.uint8)
        # elif mask.ndim == 3:
        #     mask_result = (np.argmax(mask, axis=0) * 255 / mask.shape[0]).astype(np.uint8)
        ...
        try:
            masks = self._original_predictions
